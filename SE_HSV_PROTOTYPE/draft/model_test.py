...
def verify_signature_svm(test_image_path):
    # Load and preprocess the tested image
    test_image = cv2.imread(test_image_path, cv2.IMREAD_GRAYSCALE)
    test_image = cv2.resize(test_image, (300, 300))
    test_features, hog_tested, lbp_tested = extract_features(test_image)

    # Extract features from the genuine image for visualization purposes
    _, hog_original, lbp_original = extract_features(genuine_image)
    ...
    visualize_verification(genuine_image, test_image, hog_original, hog_tested, lbp_original, lbp_tested, similarity_percentage, prediction[0])
...
# Example usage with a tested image
tested_image_path = 'signature_data/new/signature1.png'
verify_signature_svm(tested_image_path)
